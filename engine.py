...
# ============================================================
# LAYER 2 & 3: 고급 블랙리스트 패턴 (공통 악성 패턴 & 외부 데이터 전용 패턴)
# ============================================================

# ── 한국어/영어 공통 악성 패턴 (외부 데이터 & 사용자 쿼리 전체 차단) ──
COMMON_MALICIOUS_PATTERNS_KO = [
...
COMMON_MALICIOUS_PATTERNS_EN = [
    ...
    r'(?:act|run|operate|respond)\s+without\s+(?:any\s+)?(?:restrictions?|limitations?|constraints?)',
...
# ── 외부 데이터(HTML) 전용 차단 패턴 (사용자 쿼리에서는 정상 행동으로 허용) ──
EXTERNAL_ONLY_PATTERNS_KO = [
...
EXTERNAL_ONLY_PATTERNS_EN = [
    r'(?:new|override|updated?)\s+(?:instructions?|directives?|prompt)',
    r'you\s+are\s+now\s+(?:a|an|the)\s+(?:jailbroken|unrestricted|DAN|different|new|free|bot|assistant|ai|gpt|model|persona)',
    r'act\s+as\s+(?:a|an|the)\s+(?:jailbroken|unrestricted|DAN|different|new|free|bot|assistant|ai|gpt|model|persona)',
    r'pretend\s+(?:you\s+are|to\s+be)',
    r'roleplay\s+as',
]

# 구조 탈출 공격 패턴 (태그/마크다운 인젝션 - 공통)
...
class AdvancedFilterEngine:
    ...
    def __init__(self):
        ...
        self._ko_common_patterns = [re.compile(p, re.I | re.UNICODE) for p in COMMON_MALICIOUS_PATTERNS_KO]
        self._en_common_patterns = [re.compile(p, re.I | re.UNICODE) for p in COMMON_MALICIOUS_PATTERNS_EN]
        self._ko_external_patterns = [re.compile(p, re.I | re.UNICODE) for p in EXTERNAL_ONLY_PATTERNS_KO]
        self._en_external_patterns = [re.compile(p, re.I | re.UNICODE) for p in EXTERNAL_ONLY_PATTERNS_EN]
    ...
    def _scan_layer(self, normalized: str, patterns: list):
        """단일 레이어 패턴 스캔. 탐지 시 (True, match_object) 반환."""
    ...
    def _get_snippet(self, text: str, start: int, end: int, context_len: int = 30) -> str:
        """매칭된 텍스트 주변의 맥락(스니펫)을 추출"""
        start_idx = max(0, start - context_len)
        end_idx = min(len(text), end + context_len)
        prefix = "..." if start_idx > 0 else ""
        suffix = "..." if end_idx < len(text) else ""
        return prefix + text[start_idx:end_idx].replace('\n', ' ') + suffix

    def multi_layer_scan(self, text: str, logs: list, is_external: bool = True) -> tuple[bool, dict | None]:
        """
        LAYER 2~4: 다층 스캔
        반환: (is_safe, blocked_info)
        """
        normalized = self.normalize_text(text)

        # 1. 공통 악성 패턴 검사 (KO)
        logs.append(f"[Guard-L2] 한국어 악성 패턴 스캔 중... (대상: {'외부 데이터' if is_external else '사용자 쿼리'})")
        detected, match_obj = self._scan_layer(normalized, self._ko_common_patterns)
        if detected:
            snippet = self._get_snippet(normalized, match_obj.start(), match_obj.end())
            logs.append(f"[Guard-L2/KO] 🚨 악성 패턴 탐지: '{match_obj.group(0)[:40]}' (위치: {match_obj.start()})")
            return False, {
                "layer": "Guard-L2/KO",
                "pattern": match_obj.group(0),
                "snippet": snippet,
                "target": "external" if is_external else "query"
            }

        # 2. 외부 데이터 전용 한국어 패턴 검사
        if is_external:
            logs.append("[Guard-L2] 외부 데이터 전용 한국어 패턴 스캔 중...")
            detected, match_obj = self._scan_layer(normalized, self._ko_external_patterns)
            if detected:
                snippet = self._get_snippet(normalized, match_obj.start(), match_obj.end())
                logs.append(f"[Guard-L2/KO_EXT] 🚨 외부 전용 패턴 탐지: '{match_obj.group(0)[:40]}' (위치: {match_obj.start()})")
                return False, {
                    "layer": "Guard-L2/KO_EXT",
                    "pattern": match_obj.group(0),
                    "snippet": snippet,
                    "target": "external"
                }
        ...
        # 3. 공통 악성 패턴 검사 (EN)
        logs.append(f"[Guard-L3] 영어 악성 패턴 스캔 중... (대상: {'외부 데이터' if is_external else '사용자 쿼리'})")
        detected, match_obj = self._scan_layer(normalized, self._en_common_patterns)
        if detected:
            snippet = self._get_snippet(normalized, match_obj.start(), match_obj.end())
            logs.append(f"[Guard-L3/EN] 🚨 악성 패턴 탐지: '{match_obj.group(0)[:40]}' (위치: {match_obj.start()})")
            return False, {
                "layer": "Guard-L3/EN",
                "pattern": match_obj.group(0),
                "snippet": snippet,
        ...
        # 4. 외부 데이터 전용 영어 패턴 검사
        if is_external:
            logs.append("[Guard-L3] 외부 데이터 전용 영어 패턴 스캔 중...")
            detected, match_obj = self._scan_layer(normalized, self._en_external_patterns)
            if detected:
                snippet = self._get_snippet(normalized, match_obj.start(), match_obj.end())
                logs.append(f"[Guard-L3/EN_EXT] 🚨 외부 전용 패턴 탐지: '{match_obj.group(0)[:40]}' (위치: {match_obj.start()})")
                return False, {
                    "layer": "Guard-L3/EN_EXT",
                    "pattern": match_obj.group(0),
                    "snippet": snippet,
                    "target": "external"
                }
        ...
        # 5. 구조 탈출 공격 패턴
        logs.append(f"[Guard-L4] 구조 탈출(Structural Injection) 패턴 스캔 중... (대상: {'외부 데이터' if is_external else '사용자 쿼리'})")
        detected, match_obj = self._scan_layer(normalized, self._struct_patterns)
        if detected:
            snippet = self._get_snippet(normalized, match_obj.start(), match_obj.end())
            logs.append(f"[Guard-L4/STRUCT] 🚨 구조 탈출 패턴 탐지: '{match_obj.group(0)[:40]}' (위치: {match_obj.start()})")
            return False, {
                "layer": "Guard-L4/STRUCT",
                "pattern": match_obj.group(0),
                "snippet": snippet,
                "target": "external" if is_external else "query"
            }
        ...
        if is_external:
            ...
            if suspicious_ratio > 0.15:
                logs.append(f"[Guard-L5] 🚨 비정상적으로 높은 비ASCII 특수문자 비율: {suspicious_ratio:.1%} (난독화 의심)")
                snippet = text[:60] + "..." if len(text) > 60 else text
                return False, {
                    "layer": "Guard-L5/Entropy",
                    "pattern": f"high_entropy:{suspicious_ratio:.2f}",
                    "snippet": snippet,
                    "target": "external"
                }

        logs.append(f"[Guard-All] ✅ {'외부 데이터' if is_external else '사용자 쿼리'} 검사 통과. 위협 없음.")
        return True, None
...
class AntiPromptInjectionEngine:
    ...
    def dom_sanitization(self, html_content: str, logs: list) -> tuple:
        ...
        - script/style 등 본문이 제거되어 LLM에 전달되지 않는 태그는 텍스트를 수집하지 않고 완전 파기
        - iframe, object, embed, form, input 등 기타 위험 태그는 텍스트를 추출한 후 제거
        - 비시각적(display:none 등) 요소 제거 및 텍스트 추출
        - 순수 가시 텍스트 반환
        ...
        try:
            ...
            # 1. script와 style 태그는 LLM에 도달하지 않으므로 검사 대상에서 제외하고 즉시 파기 (오탐 방지 핵심)
            for tag in soup(["script", "style"]):
                tag.decompose()
                removed += 1

            # 2. 기타 위험 태그들은 텍스트를 모으고 파기
            for tag in soup(["iframe", "object", "embed", "form", "input"]):
    ...
    def process_request(self, raw_external_data: str, user_query: str):
        """
        메인 보안 파이프라인
        반환: (logs, final_result, blocked, blocked_info)
        """
        logs = []
        ...
        if hidden_text.strip():
            ...
            is_safe, block_details = self.filter.multi_layer_scan(hidden_text, logs, is_external=True)
            if not is_safe:
                logs.append("[Pipeline] 🔴 BLOCKED — 숨겨진 요소 내 악성 인젝션 탐지.")
                if block_details:
                    block_details["target"] = "hidden"
                return logs, "[CRITICAL_SECURITY_VIOLATION_DETECTED]", True, block_details

        # STEP 2: 가시 텍스트 스캔 (외부 데이터이므로 엄격 검사)
        if sanitized_data.strip():
            logs.append("[Guard-L1] 🔍 가시 텍스트(외부 데이터) 스캔 중...")
            is_safe, block_details = self.filter.multi_layer_scan(sanitized_data, logs, is_external=True)
            if not is_safe:
                logs.append("[Pipeline] 🔴 BLOCKED — 외부 데이터 내 악성 프롬프트 인젝션 차단 완료.")
                return logs, "[CRITICAL_SECURITY_VIOLATION_DETECTED]", True, block_details

        # STEP 3: 사용자 쿼리 스캔 (사용자 쿼리이므로 완화된 검사)
        if user_query.strip():
            logs.append("[Guard-L1] 🔍 사용자 쿼리(User Query) 스캔 중...")
            is_safe, block_details = self.filter.multi_layer_scan(user_query, logs, is_external=False)
            if not is_safe:
                logs.append("[Pipeline] 🔴 BLOCKED — 사용자 쿼리 내 악성 공격 패턴 감지 차단 완료.")
                return logs, "[CRITICAL_SECURITY_VIOLATION_DETECTED]", True, block_details
        ...
        if not self.output_compliance_check(mock_ai_output, logs):
            block_details = {
                "layer": "OutputGuard",
                "pattern": "script/link/api_key violation",
                "snippet": mock_ai_output[:60] + "...",
                "target": "output"
            }
            return logs, "[CRITICAL_SECURITY_VIOLATION_DETECTED]", True, block_details

        logs.append("[PIPELINE COMPLETE] 🟢 모든 보안 가드레일 통과. 결과 반환.")
        return logs, sanitized_data, False, None
